...
export default function FormsPage() {
  ...
  const [srcToResponses, setSrcToResponses] = useState<Record<string, Message[]>>({});
  ...
  useEffect(() => {
    const loadData = async () => {
      setIsFetching(true)
      // Fetch requests
      const requestsText = await fetch("/experiment/dataset/requests.csv").then(res => res.text());
      const requests = readCsvToMessages(requestsText);

      // Fetch responses
      const srcToResponses: Record<string, Message[]> = {};
      for (const src of SOURCES) {
        const responsesText = await fetch(`/experiment/results/${src}/responses.csv`).then(res => res.text());
  ...
  const canSubmit = !!fullName && !!email && Object.keys(results).length === requests.length;
            ...
            <CarouselItem key={idx} className="h-full relative">
              <SurveyForm
                ...
                srcToResponse={Object.fromEntries(
                  Object.entries(srcToResponses).map(([src, responses]) => [src, responses[idx]])
                )}
              />
            </CarouselItem>
          ))}
